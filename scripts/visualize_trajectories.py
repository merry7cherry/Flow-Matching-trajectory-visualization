...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--cheat-target-noise-ratio",
        type=float,
        default=0.0,
        help="Fraction of trajectories whose target endpoints receive noise for the cheat method",
    )
    parser.add_argument(
        "--cheat-target-noise-strength",
        type=float,
        default=0.0,
        help="Relative magnitude of noise applied to perturbed target endpoints for the cheat method",
    )
    parser.add_argument(
        "--cheat-trajectory-noise-ratio",
        type=float,
        default=0.0,
        help="Fraction of cheat trajectories that receive additional noise along the full curve",
    )
    parser.add_argument(
        "--cheat-trajectory-noise-strength",
        type=float,
        default=0.0,
        help="Relative magnitude of noise applied along cheat trajectories",
    )
    ...
    if not 0.0 <= args.cheat_target_noise_ratio <= 1.0:
        parser.error("--cheat-target-noise-ratio must be within [0, 1]")
    if args.cheat_target_noise_strength < 0.0:
        parser.error("--cheat-target-noise-strength must be non-negative")
    if not 0.0 <= args.cheat_trajectory_noise_ratio <= 1.0:
        parser.error("--cheat-trajectory-noise-ratio must be within [0, 1]")
    if args.cheat_trajectory_noise_strength < 0.0:
        parser.error("--cheat-trajectory-noise-strength must be non-negative")
    ...
    for dataset_config in selected_configs:
        ...
        if (
            ...
            cheat_trajectory = generate_cheat_ground_truth(
                ...
                target_noise_ratio=args.cheat_target_noise_ratio,
                target_noise_strength=args.cheat_target_noise_strength,
                trajectory_noise_ratio=args.cheat_trajectory_noise_ratio,
                trajectory_noise_strength=args.cheat_trajectory_noise_strength,
